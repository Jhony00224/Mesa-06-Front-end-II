let novosFelinos = [
  ...
  const container = document.querySelector(".containerFelinos");
  
  for (i = 0; i < novosFelinos.length; i++) {
    container.innerHTML += `
      <div class="item">
      <img src="${novosFelinos[i].img}">
      <h2>${novosFelinos[i].titulo}</h2>
      <p>
      ${novosFelinos[i].descricao}</p>
    </div>
      `;
  }
